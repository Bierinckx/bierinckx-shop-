...
function buildHome(t, lang) {
  ...
  ].map((b) => `<div class="brand-card"><img class="brand-img" src="${b.img}" alt="${b.name}" loading="lazy"><div class="brand-name">${b.name}</div><div class="brand-pct">${b.pct} ${lang === "nl" ? "commissie" : "commission"}</div></div>`).join("");
  const features2 = [
